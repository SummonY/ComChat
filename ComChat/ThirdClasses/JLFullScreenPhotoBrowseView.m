...
- (void)initAllViews
{
    ...
    _progressIndicator = [[MBRoundProgressView alloc] initWithFrame:CGRectMake(0.f, 0.f,
                                                                               PROGRESS_VIEW_WIDTH,
                                                                               PROGRESS_VIEW_WIDTH)];
    _progressIndicator.backgroundTintColor = [UIColor whiteColor];
...
- (void)savePhoto
{
	if (self.isOriginPhotoLoaded && self.imageView.image) {
        ...
        [self makeToast:@"图片未下载完成，无法保存" duration:1.0 position:CSToastPositionBottom];
    }
}
...
{
    if (error) {
        [self makeToast:@"保存失败" duration:1.0 position:CSToastPositionBottom];
    }
    else {
